...
RR = zeros(1,T);     % KL regret
   ...
   %   hold on
     ...
     %     plot([ss ss+1/2^ll],[yy(ii) yy(ii)],'b','linewidth',2);
         ...
         % end
          RR(tt) = RR(tt) + (-log(yy(ii))*(YY./0.1)-log(2^ll)*(YY./0.1));
         %RR(tt) = RR(tt)-log(2^ll)*YY-YY*log(yy(ii)./2^ll);
       %  tmp=[tmp,yy(ii)./2^ll];  this is  q_{at}
         %RR(tt) = RR(tt) + sum(YY./2.^ll.*log(YY./0.1./(yy(ii)./2^ll)));
        end
      end
      %RR(tt) = RR(tt)+1.1*exp(-10)-0.1*log(0.1);
     % hold off
     % drawnow
...
cumregret=cumsum(regret,2);
[min,indmin] = min(regret(:,end));

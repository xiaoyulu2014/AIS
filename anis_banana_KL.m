      ...
      while(1)   
          if dd(ii,iiy) == 0
            ...
            if ll==lly
               ...
               rr=[sum(pp(index1)) sum(pp(index2))]./[size(index1,1) size(index2,1)]  ...
                        + explorefactor*sqrt(log(nn(ii,iiy)+1))./[size(index1,1) size(index2,1)];
               q_tmp = rr/sum(rr)*q(ii,iiy);   
               timer2=toc;
               ...
               if isempty(index1)==0 & isempty(index2)==0 
                   ...
                   KL1 = sum(weights1.*log(weights1))/sum(weights1);
                   KL2 = sum(weights21.*log(weights21))/sum(weights21)+sum(weights22.*log(weights22))/sum(weights22); 
                   if log(timer1) + KL1 >  log(timer1+timer2) + KL2
                       dd(ii,iiy) = 1; %split, (ii,iiy) becomes parent node;
                       total([2*ii 2*ii+1],iiy) =  [sum(pp(index1)) sum(pp(index2))]./2*M^2/2^(ll+lly);
                       nn([2*ii 2*ii+1], iiy) = [size(index1,1) size(index2,1)];
                   end
               end       
            ...
            elseif ll>lly
               ...
               if isempty(index1)==0 && isempty(index2)==0       
                   ...
                   if log(timer1) + KL1 >  log(timer1+timer2) + KL2
                       ...
                       nn(ii,[2*iiy 2*iiy+1]) = [size(index1,1) size(index2,1)];
                   end
               end        
                % update statistics
               kk=ll;kky=lly;
     ...
     if 0
        ...
        for ii=1:size(nn,1)
            for jj=1:size(nn,2)
                 ...
                 elseif  nn(ii,jj)>0 && ( (ll==lly && dd(ii,jj)==0) || ((ll==lly+1 && dd(ii,jj)==0)) )
